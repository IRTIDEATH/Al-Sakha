...
const WorkExperienceSection = () => {
  return (
    <section>
      ...
      <p className="mb-8 w-full text-start font-medium text-[16px] text-zeta">
      ...
      <div className="flex flex-col gap-6">
        {internships.map((item, index) => (
          <div
            key={index}
            className="flex w-full flex-col items-start sm:flex-row sm:items-end sm:justify-between"
          >
            <div className="flex flex-col gap-0.5">
              <Link
                ...
                target="_blank"
                href={item.link}
                className="group inline-flex w-fit items-end gap-1 font-medium font-roboto text-xl"
              >
                {item.company}
              ...
              <p className="font-medium text-lg text-zeta">{item.position}</p>
            </div>
            <span className="mt-2 font-medium text-[14px] text-zeta">
              {item.date}
            </span>
          </div>
        ))}
